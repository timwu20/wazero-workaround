...
func NewAllocator(mem api.Memory, ptrOffset uint32) *FreeingBumpHeapAllocator {
...
func (fbha *FreeingBumpHeapAllocator) growHeap(numPages uint32, heap api.Memory) error {
	_, ok := heap.Grow(numPages)
	if !ok {
		return fmt.Errorf("heap.Grow ignored")
	}

	fbha.maxHeapSize = heap.Size() - alignment
	return nil
}
...
func (fbha *FreeingBumpHeapAllocator) Allocate(size uint32, mem api.Memory) (uint32, error) {
	...
	if (itemSize + fbha.totalSize + fbha.ptrOffset) > fbha.maxHeapSize {
		...
		err := fbha.growHeap(pagesNeeded+1, mem)
		if err != nil {
	...
	if item := fbha.heads[listIndex]; item != 0 {
		...
		fourBytes := fbha.getHeap4bytes(item, mem)
	...
	for i := uint32(1); i <= 8; i++ {
		fbha.setHeap(ptr-i, 255, mem)
	}
	fbha.setHeap(ptr-8, uint8(listIndex), mem)
...
func (fbha *FreeingBumpHeapAllocator) Deallocate(pointer uint32, mem api.Memory) error {
	ptr := pointer - fbha.ptrOffset
	if ptr < 8 {
		return errors.New("invalid pointer for deallocation")
	}
	listIndex := fbha.getHeapByte(ptr-8, mem)
	...
	fbha.setHeap4bytes(ptr-8, bTail, mem)
...
func (fbha *FreeingBumpHeapAllocator) setHeap(ptr uint32, value uint8, heap api.Memory) {
	if !heap.WriteByte(fbha.ptrOffset+ptr, value) {
		panic("write: out of range")
	}
}

func (fbha *FreeingBumpHeapAllocator) setHeap4bytes(ptr uint32, value []byte, heap api.Memory) {
	if !heap.Write(fbha.ptrOffset+ptr, value) {
		panic("write: out of range")
	}
}

func (fbha *FreeingBumpHeapAllocator) getHeap4bytes(ptr uint32, heap api.Memory) []byte {
	bytes, ok := heap.Read(fbha.ptrOffset+ptr, 4)
	if !ok {
...
func (fbha *FreeingBumpHeapAllocator) getHeapByte(ptr uint32, heap api.Memory) byte {
	b, ok := heap.ReadByte(fbha.ptrOffset + ptr)
	if !ok {
		panic("read: out of range")
	}
	return b
